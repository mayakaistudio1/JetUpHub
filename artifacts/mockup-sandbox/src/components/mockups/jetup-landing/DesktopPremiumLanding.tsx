...
export function DesktopPremiumLanding() {
  ...
  return (
    <div style={{ background: BG, color: TEXT_PRIMARY, fontFamily: "'Montserrat', sans-serif", overflowX: "hidden", minHeight: "100vh" }}>
      ...
      <header style={{
        ...
        background: "rgba(10,10,18,0.6)", backdropFilter: "blur(40px)",
      ...
      <motion.section
        ref={heroRef}
        style={{ opacity: heroOpacity, scale: heroScale, minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 80px", position: "relative", overflow: "hidden" }}
      >
        <div style={{ position: "absolute", top: "30%", left: "-5%", width: "45vw", height: "45vw", background: `radial-gradient(circle, ${ACCENT}08 0%, transparent 65%)`, borderRadius: "50%", filter: "blur(120px)", pointerEvents: "none" }} />
        ...
        >
          <motion.div
            ...
            style={{ fontSize: 14, fontWeight: 600, color: ACCENT_LIGHT, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 40 }}
          >
          ...
          <motion.h1
            ...
            style={{ fontSize: 86, fontWeight: 800, lineHeight: 1.04, letterSpacing: "-0.04em", marginBottom: 40 }}
          >
            This is not just<br />another opportunity.
          </motion.h1>
          ...
          >
            ...
            <span style={{ fontSize: 32, fontWeight: 300, color: TEXT_SECONDARY, letterSpacing: "-0.02em" }}>
              This is a <span style={{ color: TEXT_PRIMARY, fontWeight: 600 }}>system.</span>
            </span>
          </motion.div>
          ...
          <motion.p
            ...
            style={{ fontSize: 18, color: TEXT_SECONDARY, maxWidth: 560, lineHeight: 1.7, marginTop: 56, fontWeight: 400 }}
          >
            Product at the center. Partner model for scale.
        ...
        >
          <span style={{ fontSize: 12, color: TEXT_MUTED, letterSpacing: "0.15em", textTransform: "uppercase" }}>Explore</span>
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} style={{ width: 1, height: 32, background: `linear-gradient(180deg, ${TEXT_MUTED}, transparent)` }} />
        </motion.div>
      </motion.section>
      ...
      <section style={{ padding: "80px 80px", maxWidth: 1200, margin: "0 auto" }}>
        <FadeIn>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 20, fontWeight: 600, letterSpacing: "0.08em", color: TEXT_MUTED }}>
        ...
        <FadeIn delay={0.3}>
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, marginTop: 32 }}>
              {[
                { title: "Digital Hub", desc: "Unified entry point with AI-guided navigation" },
                { title: "AI Duplication", desc: "Clone the leader's communication style at scale" },
                { title: "Automated Onboarding", desc: "First steps guided without leader's time" },
              ].map(item => (
                <div key={item.title} style={{ padding: 32, borderRadius: 16, border: `1px solid ${BORDER}` }}>
                  <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>{item.title}</div>
                  <div style={{ fontSize: 14, color: TEXT_SECONDARY, lineHeight: 1.6 }}>{item.desc}</div>
                </div>
              ))}
            </div>
          </PillarScene>
      ...
      <section style={{ padding: "160px 80px", maxWidth: 1200, margin: "0 auto" }}>
        <FadeIn>
          <div style={{ textAlign: "center", maxWidth: 800, margin: "0 auto" }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: TEXT_MUTED, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 32 }}>
              Transformation
            </div>
            <h2 style={{ fontSize: 52, fontWeight: 800, lineHeight: 1.15, letterSpacing: "-0.04em", marginBottom: 40 }}>
              From random growth<br />to managed infrastructure.
            </h2>
            <p style={{ fontSize: 20, color: TEXT_SECONDARY, lineHeight: 1.7, marginBottom: 64 }}>
              JetUP exists to replace dependence on chaos, randomness, and an overloaded leader — with a system where the client gets value, the partner builds income, and the leader scales through modern infrastructure.
            </p>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.2}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, borderRadius: 20, overflow: "hidden" }}>
            {[
              ...
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", background: SURFACE, padding: "32px 40px", alignItems: "center", gap: 24 }}>
                <span style={{ fontSize: 16, color: TEXT_MUTED, textDecoration: "line-through", textDecorationColor: "rgba(255,255,255,0.15)" }}>{row.before}</span>
                <span style={{ color: ACCENT_LIGHT, fontSize: 14 }}>→</span>
                <span style={{ fontSize: 16, fontWeight: 600 }}>{row.after}</span>
              </div>
            ))}
          </div>
        </FadeIn>
      ...
      {/* ═══════ CTA ═══════ */}
      <section style={{ padding: "120px 80px 160px", maxWidth: 1200, margin: "0 auto" }}>
        <FadeIn>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: ACCENT_LIGHT, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 32 }}>
              Ready?
            </div>
            <h2 style={{ fontSize: 64, fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.04em", marginBottom: 24 }}>
              Enter the system.
            </h2>
            <p style={{ fontSize: 18, color: TEXT_SECONDARY, marginBottom: 56, lineHeight: 1.6, maxWidth: 500, margin: "0 auto 56px" }}>
              Talk to Maria. Explore at your pace.<br />Find your path inside JetUP.
            </p>
            <div style={{ display: "flex", justifyContent: "center", gap: 16 }}>
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                style={{
                  padding: "18px 48px", background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT_LIGHT})`,
                  border: "none", borderRadius: 100, color: "#fff", fontSize: 16, fontWeight: 700,
              ...
              <motion.button
                ...
                style={{
                  ...
                  border: `1px solid ${BORDER}`, borderRadius: 100, color: TEXT_PRIMARY, fontSize: 16, fontWeight: 600,
                  cursor: "pointer", fontFamily: "'Montserrat', sans-serif", letterSpacing: "0.02em",
                }}
              >
                Watch Presentation
      ...
      {/* ═══════ FOOTER ═══════ */}
      <footer style={{ padding: "48px 80px", borderTop: `1px solid ${BORDER}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 14, fontWeight: 700, letterSpacing: "0.08em", color: TEXT_MUTED }}>JETUP</div>
        <div style={{ fontSize: 13, color: TEXT_MUTED }}>Digital Infrastructure Platform</div>
      </footer>
    </div>
  );
}
