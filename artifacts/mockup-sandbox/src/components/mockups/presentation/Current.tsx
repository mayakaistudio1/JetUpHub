...
function Timeline({ currentIndex, onSelect }: { currentIndex: number; onSelect: (i: number) => void }) {
  return (
    <div className="absolute top-0 left-0 right-0 z-40 px-3 pt-4 pb-2">
...
function Subtitles({ texts, currentTime }: { texts: TextOverlay[]; currentTime: number }) {
  ...
  return (
    <div className="absolute left-0 right-0 z-20 flex flex-col items-center gap-1 px-6" style={{ bottom: "10%" }}>
      {activeTexts.map((t, i) => (
        <p
          key={`${t.text}-${i}`}
...
function PremiumCard({ scene, onNext }: { scene: SceneConfig; onNext: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-xl animate-card-in overflow-y-auto max-h-[80vh] no-scrollbar" style={{ scrollbarWidth: "none" }}>
        ...
        >
          ...
          {scene.cardSubtitle && (
            <p className="text-center text-white/50 text-sm md:text-base mb-6 font-light tracking-wide">
              {scene.cardSubtitle}
            </p>
          )}
              ...
              {scene.subBlocks!.map((block, i) => {
                ...
                return (
                  <div key={i} className="group">
                    ...
                    {isOpen && (
                      <div className="px-4 pt-3 pb-1 animate-block-expand">
...
export function Current() {
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      <style>{`
